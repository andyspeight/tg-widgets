...
// Honour a <base href> if the page has one, as a browser would.
const baseTag = (html.match(/<base\b[^>]*href\s*=\s*["']([^"']+)["']/i) || [])[1];
const base = baseTag ? new URL(decode(baseTag), res.url || url).href : (res.url || url);
const links = [...html.replace(/<!--[\s\S]*?-->/g, ' ').matchAll(/<a\b[^>]*?href\s*=\s*["']([^"'#][^"']*)["'][^>]*>([\s\S]*?)<\/a>/gi)]
  .map(m => [new URL(decode(m[1]), base).href, decode(m[2].replace(/<[^>]+>/g, ' ')).replace(/\s+/g, ' ').trim()]);
