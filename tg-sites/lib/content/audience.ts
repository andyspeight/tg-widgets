...
// The editor's country names live in lib/content/countries.ts (the full ISO list,
// editor-only), so the render bundle carries codes, not names. The rule here
// validates a country by SHAPE, never against a name list.
const COUNTRY = /^[A-Z]{2}$/;
