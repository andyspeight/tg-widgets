...
assert(html.includes('Skip the queue') && html.includes('Premier Access'), 'guides: fast track');
assert(html.includes('Tickets and prices') && html.includes('from around £60'), 'tickets');
assert(html.includes('Nearest airports') && html.includes('CDG'), 'getting there: airports');
assert(html.includes('Google Maps') && html.includes('maps.apple.com'), 'map links from lat/lng');
...
assert(!html2.includes('Google Maps'), 'no map links without lat/lng');
