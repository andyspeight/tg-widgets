...
(function () {
  ...
  var DEFAULTS = {
    ...
    theme: 'light',
    accent: '#00B4D8',
    bookTextColor: '',         // Book-button text; '' = auto dark-on-accent
    radius: 12,
    fontFamily: '',
    appId: '',
  };
  ...
  function stayOpen(w, btn) {
    ...
    function draw() {
      ...
      var html = '<div class="tgcp-fly-t">When would you like to stay?</div>'
        + '<div class="tgcp-stay-head">'
        + '<button type="button" class="tgcp-stay-nav" data-nav="-1"' + (canPrev ? '' : ' disabled')
        + ' aria-label="Previous month">&lsaquo;</button>'
        + '<span class="tgcp-stay-month">' + esc(STAY_MONTHS[mo] + ' ' + y) + '</span>'
        + '<button type="button" class="tgcp-stay-nav" data-nav="1"' + (canNext ? '' : ' disabled')
        + ' aria-label="Next month">&rsaquo;</button>'
        + '</div><div class="tgcp-stay-grid">';
      for (var i = 0; i < 7; i++) html += '<span class="tgcp-stay-dow">' + STAY_DAYS[i] + '</span>';
      for (var b = 0; b < startCol; b++) html += '<span></span>';
      for (var day = 1; day <= dim; day++) {
        var d = new Date(y, mo, day);
        var ok = state.checkIn ? (canOut(d) || canIn(d)) : canIn(d);
        var cls = 'tgcp-stay-day';
      ...
      html += '</div><div class="tgcp-fly-note">' + esc(pkg
        ...
        + (pkg ? '<button type="button" class="tgcp-stay-go">Choose airport &middot; '
    ...
    box.addEventListener('click', function (e) {
      ...
      var goBtn = e.target && e.target.closest ? e.target.closest('.tgcp-stay-go') : null;
  ...
  var STAY_CSS = '.tgcp-stay-head{display:flex;align-items:center;justify-content:space-between;margin:2px 0 6px}'
    + '.tgcp-stay-month{font-weight:600;font-size:13px}'
    + '.tgcp-stay-nav{width:28px;height:28px;border:0;background:none;font:inherit;font-size:16px;'
    + 'color:inherit;cursor:pointer;border-radius:8px}'
    + '.tgcp-stay-nav:hover{background:rgba(0,0,0,.07)}'
    + '.tgcp-stay-nav[disabled]{opacity:.3;cursor:default;background:none}'
    + '.tgcp-stay-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;text-align:center}'
    + '.tgcp-stay-dow{font-size:10px;font-weight:600;color:#5b6b7b;text-transform:uppercase;'
    + 'letter-spacing:.04em;padding:2px 0}'
    + '.tgcp-stay-day{border:0;background:none;font:inherit;font-size:12.5px;color:inherit;padding:0;'
    + 'height:32px;border-radius:8px;cursor:pointer}'
    + '.tgcp-stay-day:hover{background:rgba(0,0,0,.07)}'
    + '.tgcp-stay-day.is-off{opacity:.28;cursor:default}'
    + '.tgcp-stay-day.is-off:hover{background:none}'
    + '.tgcp-stay-day.is-event{box-shadow:inset 0 0 0 1.5px currentColor}'
    + '.tgcp-stay-day.is-pick{background:#1a2733;color:#fff}'
    + '.tgcp-stay-day.is-span{background:rgba(0,0,0,.08)}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-dow{color:#93a4b5}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-nav:hover{background:rgba(255,255,255,.09)}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-day:hover{background:rgba(255,255,255,.09)}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-day.is-off:hover{background:none}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-day.is-pick{background:#e8eef4;color:#16202c}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-day.is-span{background:rgba(255,255,255,.1)}'
    + '.tgcp-stay-go{display:block;width:100%;margin-top:8px;padding:9px 10px;border:0;border-radius:8px;'
    + 'font:inherit;font-size:13px;font-weight:600;cursor:pointer;background:#1a2733;color:#fff}'
    + '.tgcp-stay-go:hover{filter:brightness(1.15)}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-go{background:#e8eef4;color:#16202c}'
    + '.tgcp-root[data-theme="dark"] .tgcp-stay-go:hover{filter:brightness(.93)}';
  ...
  function styles(cfg) {
    ...
    return ':host{all:initial;display:block;width:100%;min-width:0;}:host::before{content:"";display:block;width:280px;max-width:100%;height:0;}'
      ...
      + '.tgcp-btn{font-weight:700;letter-spacing:.01em;'
      + 'box-shadow:0 2px 6px color-mix(in srgb,var(--tgcp-accent) 34%,transparent);'
