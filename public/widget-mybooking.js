 ...
 *     order.money): payments received, then each gift voucher as a credit
 *     with its name and a masked code, the balance (total less payments less
 *     voucher credit, never below zero), what is payable now, and a status
 *     line. A booking settled by a voucher shows a zero balance and no Pay
 *     button. Order TG120193 (£9.17 holiday, £9.17 gift voucher) used to show
 *     £9.17 still due.
 *
 * v1.10.0 changes:
...
(function () {
  ...
  const MESSAGES = {
    en: {
      ...
      dueNow: 'Due now',
      voucher: 'Voucher',
      payStatusSettled: 'Your booking is paid in full. No further payment is due.',
      payStatusPartly: 'Your booking is secured. The remaining {amount} is due before you travel.',
    ...
    fr: {
      ...
      amountPayableNow: 'Montant à régler maintenant',
      dueNow: 'À régler maintenant',
      voucher: 'Bon',
      payStatusSettled: 'Votre réservation est intégralement payée. Aucun autre paiement n\'est dû.',
      payStatusPartly: 'Votre réservation est confirmée. Le solde de {amount} est à régler avant le départ.',
    ...
    de: {
      ...
      amountPayableNow: 'Jetzt fälliger Betrag',
      dueNow: 'Jetzt fällig',
      voucher: 'Gutschein',
      payStatusSettled: 'Ihre Buchung ist vollständig bezahlt. Es ist keine weitere Zahlung fällig.',
      payStatusPartly: 'Ihre Buchung ist gesichert. Der Restbetrag von {amount} ist vor der Abreise fällig.',
    ...
    es: {
      ...
      amountPayableNow: 'Importe a pagar ahora',
      dueNow: 'Vence ahora',
      voucher: 'Vale',
      payStatusSettled: 'Su reserva está pagada en su totalidad. No queda ningún pago pendiente.',
      payStatusPartly: 'Su reserva está asegurada. El saldo restante de {amount} debe abonarse antes del viaje.',
    ...
    it: {
      ...
      amountPayableNow: 'Importo da pagare ora',
      dueNow: 'Da pagare ora',
      voucher: 'Buono',
      payStatusSettled: 'La sua prenotazione è pagata per intero. Non è dovuto alcun altro pagamento.',
      payStatusPartly: 'La sua prenotazione è confermata. Il saldo residuo di {amount} è dovuto prima della partenza.',
    ...
    ro: {
      ...
      amountPayableNow: 'Suma de plată acum',
      dueNow: 'Scadent acum',
      voucher: 'Voucher',
      payStatusSettled: 'Rezervarea dumneavoastră este plătită integral. Nu mai este necesară nicio plată.',
      payStatusPartly: 'Rezervarea dumneavoastră este asigurată. Soldul rămas de {amount} trebuie achitat înainte de călătorie.',
  ...
  // A booking date is read as the wall clock wrote it, on every device.
  // bookingMoment (see the order-stays core above) hands back a Date whose UTC
  // fields are the numbers in the string, so pinning the display to UTC prints
  // the supplier's own date. Without the pin, a customer in New York opening
  // this page saw a check-out of 1 Oct where the paperwork said 2 Oct, because
  // a date-only value is midnight UTC and their evening is the day before.
  ...
  // Travelify dresses airport-local times as UTC, so the UTC components ARE
  // the clock the traveller reads at the gate. That only held while the value
  // was parsed as UTC: new Date('2026-10-02T14:00:00') is parsed in the
  // READER's timezone, so a 14:00 flight printed 13:00 in British Summer Time
  // and 18:00 in New York (15 Sep 2026, found alongside ET121109).
  ...
  const STYLES = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .tgm-pay-row.tgm-pay-summary { margin-top: 6px; padding-top: 12px; border-top: 1px solid var(--tgm-border-light); }
    .tgm-pay-row.tgm-pay-summary .tgm-pay-label { font-weight: 600; color: var(--tgm-text); }
    .tgm-pay-row.tgm-pay-summary .v { font-weight: 700; }
    .tgm-pay-status { font-size: 13px; line-height: 1.5; color: var(--tgm-text-2); padding: 8px 0 0; }
    .tgm-pay-status.is-settled { color: var(--tgm-success); font-weight: 500; }
  ...
  // ----- Money: the ONE calculation of what the booking still owes -----
  //
  // This block is a verbatim copy of the core of public/_order-money.js, the
  // module the booking PDF, the confirmation email and /api/pay-balance use.
  // The widget is a single script on customer sites and cannot import, so the
  // copy is held in step by test/order-money-drift-smoke.mjs. Edit the module,
  // then copy the block; never edit here alone.
  //
  // In practice the widget rarely computes anything: /api/retrieve-order runs
  // the same code on the raw Travelify order and attaches the result as
  // order.money, which orderMoney() below prefers. The copy is for a sample
  // order that has none.
  ...
  function renderPayBalance(order, currency, c) {
    ...
    if (!(outstanding > 0)) return ''; // nothing owed → no CTA (e.g. fully paid, or settled by a voucher)
  ...
  function renderFound(order, c, lookup) {
    ...
    const checkin = stays[0]?.checkin || accItem?.startDate || null;
    // The old single-stay check-out worked itself out here, in milliseconds
    // from a locally parsed date. Every card reads listStays now, so this is
    // the last copy of that arithmetic and it is gone rather than left lying
    // about waiting to be used again.
      ...
      <div class="tgm-found">
        ...
        <div class="tgm-two">
          <div class="tgm-section">
            ...
            ${(() => {
              // Order-level payment state from the ONE shared calculation
              // (/api/retrieve-order attaches it as order.money): payments
              // received, then each voucher as a credit, then the balance,
              // then what is payable now, then a plain status line. A voucher
              // that settles the booking shows a zero balance, not a demand.
              const money = orderMoney(order);
              if (money.status === 'none') return '';
              const { paid, balance } = money;
              if (!(paid > 0) && !(balance > 0) && !money.vouchers.length) return '';
              let rows = '';
              if (paid > 0) {
              ...
              money.vouchers.forEach((v) => {
                rows += `<div class="tgm-pay-row tgm-pay-voucher"><span class="tgm-pay-label">${esc(voucherLabel(v, c.labels?.voucher || c.t('voucher')))}</span><span class="v paid">${esc(fmtMoney(-v.credit, currency))}</span></div>`;
              });
              rows += `<div class="tgm-pay-row"><span class="tgm-pay-label">${esc(c.labels?.balanceRemaining || c.t('balanceRemaining'))}</span><span class="v ${balance > 0 ? 'due' : 'paid'}">${esc(fmtMoney(balance, currency))}</span></div>`;
              if (balance > 0) {
                const next = money.nextDue;
                const sched = money.schedule;
                // Instalment plan: show the whole remaining schedule (unless the
                ...
                if (next && next.isInstalment && !installPlan && sched.length > 1) {
                  rows += `<div class="tgm-pay-sched">
                    ...
                    ${sched.map((b) => {
                      ...
                      const when = b.isInitial ? (c.labels?.dueNow || c.t('dueNow')) : fmtDate(b.dueDate);
                      return `
                      <div class="tgm-inst${b.isDue ? ' is-next' : ''}">
                        <span class="date">${esc(when)}${b.isDue ? `<span class="tgm-inst-tag">${esc(c.labels?.next || c.t('next'))}</span>` : ''}</span>
              ...
              rows += `<div class="tgm-pay-row tgm-pay-summary"><span class="tgm-pay-label">${esc(c.labels?.amountPayableNow || c.t('amountPayableNow'))}</span><span class="v ${balance > 0 ? 'due' : 'paid'}">${esc(fmtMoney(money.payable, currency))}</span></div>`;
              const statusLine = money.status === 'settled'
                ? (c.labels?.payStatusSettled || c.t('payStatusSettled'))
                : (money.status === 'partly' ? c.t('payStatusPartly', { amount: fmtMoney(balance, currency) }) : '');
              if (statusLine) rows += `<div class="tgm-pay-status${money.settled ? ' is-settled' : ''}">${esc(statusLine)}</div>`;
              return rows;
            })()}
            ${standardDep ? `
