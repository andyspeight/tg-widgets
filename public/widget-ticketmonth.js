...
(function () {
  ...
  function stayOpen(w, btn) {
    ...
    function draw() {
      ...
      var html = '<div class="tgtm-fly-t">When would you like to stay?</div>'
        + '<div class="tgtm-stay-head">'
        + '<button type="button" class="tgtm-stay-nav" data-nav="-1"' + (canPrev ? '' : ' disabled')
        + ' aria-label="Previous month">&lsaquo;</button>'
        + '<span class="tgtm-stay-month">' + esc(STAY_MONTHS[mo] + ' ' + y) + '</span>'
        + '<button type="button" class="tgtm-stay-nav" data-nav="1"' + (canNext ? '' : ' disabled')
        + ' aria-label="Next month">&rsaquo;</button>'
        + '</div><div class="tgtm-stay-grid">';
      for (var i = 0; i < 7; i++) html += '<span class="tgtm-stay-dow">' + STAY_DAYS[i] + '</span>';
      for (var b = 0; b < startCol; b++) html += '<span></span>';
      for (var day = 1; day <= dim; day++) {
        var d = new Date(y, mo, day);
        var ok = state.checkIn ? (canOut(d) || canIn(d)) : canIn(d);
        var cls = 'tgtm-stay-day';
      ...
      html += '</div><div class="tgtm-fly-note">' + esc(pkg
        ...
        + (pkg ? '<button type="button" class="tgtm-stay-go">Choose airport &middot; '
    ...
    box.addEventListener('click', function (e) {
      ...
      var goBtn = e.target && e.target.closest ? e.target.closest('.tgtm-stay-go') : null;
  ...
  var STAY_CSS = '.tgtm-stay-head{display:flex;align-items:center;justify-content:space-between;margin:2px 0 6px}'
    + '.tgtm-stay-month{font-weight:600;font-size:13px}'
    + '.tgtm-stay-nav{width:28px;height:28px;border:0;background:none;font:inherit;font-size:16px;'
    + 'color:inherit;cursor:pointer;border-radius:8px}'
    + '.tgtm-stay-nav:hover{background:rgba(0,0,0,.07)}'
    + '.tgtm-stay-nav[disabled]{opacity:.3;cursor:default;background:none}'
    + '.tgtm-stay-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;text-align:center}'
    + '.tgtm-stay-dow{font-size:10px;font-weight:600;color:#5b6b7b;text-transform:uppercase;'
    + 'letter-spacing:.04em;padding:2px 0}'
    + '.tgtm-stay-day{border:0;background:none;font:inherit;font-size:12.5px;color:inherit;padding:0;'
    + 'height:32px;border-radius:8px;cursor:pointer}'
    + '.tgtm-stay-day:hover{background:rgba(0,0,0,.07)}'
    + '.tgtm-stay-day.is-off{opacity:.28;cursor:default}'
    + '.tgtm-stay-day.is-off:hover{background:none}'
    + '.tgtm-stay-day.is-event{box-shadow:inset 0 0 0 1.5px currentColor}'
    + '.tgtm-stay-day.is-pick{background:#1a2733;color:#fff}'
    + '.tgtm-stay-day.is-span{background:rgba(0,0,0,.08)}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-dow{color:#93a4b5}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-nav:hover{background:rgba(255,255,255,.09)}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-day:hover{background:rgba(255,255,255,.09)}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-day.is-off:hover{background:none}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-day.is-pick{background:#e8eef4;color:#16202c}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-day.is-span{background:rgba(255,255,255,.1)}'
    + '.tgtm-stay-go{display:block;width:100%;margin-top:8px;padding:9px 10px;border:0;border-radius:8px;'
    + 'font:inherit;font-size:13px;font-weight:600;cursor:pointer;background:#1a2733;color:#fff}'
    + '.tgtm-stay-go:hover{filter:brightness(1.15)}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-go{background:#e8eef4;color:#16202c}'
    + '.tgtm-root[data-theme="dark"] .tgtm-stay-go:hover{filter:brightness(.93)}';

  var FLY_CSS = '.tgtm-root{position:relative}'
    + 'button.tgtm-btn{appearance:none;-webkit-appearance:none;margin:0}'
  ...
  function styles(cfg) {
    ...
    return ':host{all:initial;display:block;width:100%;min-width:0;}:host::before{content:"";display:block;width:280px;max-width:100%;height:0;}'
      ...
      + '.tgtm-btn svg{width:20px;height:20px;}'
