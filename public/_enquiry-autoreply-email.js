// =============================================================================
//  /public/_enquiry-autoreply-email.js — shared server + editor module
// =============================================================================
//
//  Lives in public/ so it is BOTH the server renderer (imported by
//  api/enquiry/_lib/routing/auto-reply.js through the _templates shim) and the
//  Enquiry editor's live preview (imported in the browser as an ES module).
//  One module, so the wording a client previews is the wording their customer
//  receives. Keep it dependency-free and runtime-neutral: no Node imports, no
//  DOM access.
//
...
//
//  A client can also write their OWN message in plain prose (t.customMessage).
//  That replaces our fixed "What happens next" three-step block and keeps
//  everything else — their branded hero, the recap of what the visitor told
//  us, the Luna card and the sign-off. Prose is escaped and paragraph-split
//  here, so no client HTML ever reaches a customer inbox by that route.
//
...
/**
 * Turn a client's plain-text message into escaped HTML paragraphs. A blank
 * line starts a new paragraph, a single newline is a line break. Mirrors
 * bodyToHtml in _reminder-email-template.js so both editors behave the same.
 */
function messageToHtml(message) {
  return String(message == null ? '' : message)
    ...
    .map(b => b.trim())
    .filter(Boolean)
    .map(b => `<p style="margin:0 0 14px;color:#475569;font-size:14px;line-height:1.65;">${esc(b).replace(/\n/g, '<br>')}</p>`)
    .join('');
}
...
export function renderDefaultAutoReplyEmail(t) {
  ...
  // A client's own message replaces our fixed "what happens next" steps.
  const customMessageHtml = messageToHtml(t.customMessage);
    ...
    <tr>
      <td align="center" style="padding:32px 16px;">
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 1px 3px rgba(15,23,42,0.06);">
          ...
          ${customMessageHtml ? `
          <!-- The client's own message, in place of our "what happens next" -->
          <tr>
            <td style="padding:28px 32px 8px;">
              ${customMessageHtml}
            </td>
