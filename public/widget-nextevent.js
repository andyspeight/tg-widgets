...
(function () {
  ...
  function stayOpen(w, btn) {
    ...
    function draw() {
      ...
      html += '</div><div class="tgne-fly-note">' + esc(pkg
        ...
        + (pkg ? '<button type="button" class="tgne-stay-go">Choose airport &middot; '
    ...
    box.addEventListener('click', function (e) {
      ...
      var goBtn = e.target && e.target.closest ? e.target.closest('.tgne-stay-go') : null;
  ...
  var STAY_CSS = '.tgne-stay-head{display:flex;align-items:center;justify-content:space-between;margin:2px 0 6px}'
    ...
    + '.tgne-stay-go{display:block;width:100%;margin-top:8px;padding:9px 10px;border:0;border-radius:8px;'
    + 'font:inherit;font-size:13px;font-weight:600;cursor:pointer;background:#1a2733;color:#fff}'
    + '.tgne-stay-go:hover{filter:brightness(1.15)}'
    + '.tgne-root[data-theme="dark"] .tgne-stay-go{background:#e8eef4;color:#16202c}'
    + '.tgne-root[data-theme="dark"] .tgne-stay-go:hover{filter:brightness(.93)}';
