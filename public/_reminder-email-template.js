/**
 * Payment reminder email — renderer (shared server + editor module).
 *
 * Lives in public/ so it is BOTH the server renderer (imported by the cron
 * send worker via the api/_lib/payment-reminder-email.js shim) and the My
 * Booking editor's popup preview (imported in the browser as an ES module,
 * like _pdf-template.js / _email-template.js). One module, so the preview a
 * client sees in the editor is byte-for-byte the email that gets sent. Keep
 * it dependency-free and runtime-neutral: no Node imports, no DOM access.
 *
