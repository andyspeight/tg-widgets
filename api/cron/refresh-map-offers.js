...
// DEAD END, recorded so it is not retried: fetching a WIDER band per hotel
// request (maxOffers 1000, to reach the dearer cities buried below the cheapest
// slice) made EVERY Accommodation request FAIL — a 1,000-room search does not
// come back inside the 10s per-request timeout, so the sweep stored zero hotels
// and the fail-safe kept the stale cache, which read as "no change, still 464".
// Packages and flights in the SAME run were fine at 250. Hotels therefore stay
// at the 250 default; spreading them beyond the cheapest cities has to come from
// several SMALL per-city requests (Andy's "location names"), never one large slow
// one. (Andy, Aug 2026 — verified in the map-cron sweep logs: 12/12 hotel
// requests failed across four countries the moment the band went to 1,000.)
...
function buildPayload(row, destinationCode, market = MARKETS[0], sweepType = SWEEP_TYPES[0], flightOrigin = null, currency = 'GBP') {
  ...
  return {
    ...
    // One request size for every type. Hotels must NOT ask for more (a big
    // hotel search times out — see the ACCOMMODATION dead-end note above).
