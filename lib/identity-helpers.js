...
export async function requireOwner(req, res) {
  ...
  //
  // STRATEGY: try a sequence of progressively-broader queries so that if the
  // Permissions table uses slightly different field names than we expect, we
  // still find the right row. Worst case, we pull the user's permissions by
  // userId only and check the role+product client-side.
  try {
    // Strategy 1: pull all permissions where the User link matches.
    // This is the most schema-tolerant — we only need a {User} field on the table.
    const filter = `FIND('${userRecordId}', ARRAYJOIN({User}))`;
    const permsRes = await airtableFetch(
      `${T_PERMISSIONS}?filterByFormula=${encodeURIComponent(filter)}&maxRecords=50`
    );

    // Now inspect the rows client-side. We're looking for a row where:
    //   - the product is widget_suite (could be in {Product Slug}, {Slug}, {Product}, or a lookup)
    //   - the role is owner
    //   - the status is Active (or empty/missing — treat that as Active)
    const owns = (permsRes.records || []).some((r) => {
      ...
      if (role !== 'owner' || status !== 'active') return false;

      // Resolve product slug from any of the plausible field shapes
      const candidateSlugs = [
        ...(Array.isArray(fields['Product Slug']) ? fields['Product Slug'] : [fields['Product Slug']]),
        ...(Array.isArray(fields['Slug']) ? fields['Slug'] : [fields['Slug']]),
        ...(Array.isArray(fields['Product Name']) ? fields['Product Name'] : []),
      ].filter(Boolean).map((s) => String(s).toLowerCase());

      return candidateSlugs.some((s) => s === 'widget_suite' || s === 'tg widget suite' || s === 'widget suite');
    });

    if (!owns) {
      // Either no permission rows, or none match widget_suite + owner + active.
      // Help debugging by including a hint about what WAS found.
      const summary = (permsRes.records || []).slice(0, 5).map((r) => ({
        role: r.fields.Role,
        status: r.fields.Status,
        productSlug: r.fields['Product Slug'] || r.fields['Slug'] || null,
        productName: r.fields['Product Name'] || null,
      }));
      console.error('[requireOwner] no widget_suite owner perm for', userRecordId, 'found:', JSON.stringify(summary));
      res.status(403).json({
        ok: false,
        code: 'forbidden',
        error: 'Admin access only',
        // Safe to expose: we only show this AFTER cookie auth succeeded, so the
        // request is already authenticated. Helps Andy debug schema mismatches.
        debug: { permissionRowsFound: permsRes.records?.length || 0, userRecordId, summary },
      });
      return null;
    }
    ...
    console.error('[requireOwner] permission check failed:', err && err.message, err && err.status);
    res.status(500).json({
      ok: false,
      error: 'Failed to verify access',
      // Same safety reasoning: only emitted after cookie auth succeeded.
      // Lets us see exactly what Airtable returned without needing log access.
      debug: {
        message: err && err.message ? String(err.message).slice(0, 500) : 'unknown',
        airtableStatus: err && err.status ? err.status : null,
        userRecordId,
      },
    });
    return null;
